...
static ppa_client_handle_t s_fill;
static ppa_client_handle_t s_blend;
static ppa_client_handle_t s_srm;
static bool s_fill_error_logged;
static bool s_blend_error_logged;
static bool s_srm_error_logged;
...
{
    if (s_fill != NULL && s_blend != NULL && s_srm != NULL) {
        return ESP_OK;
    }
    if (s_fill != NULL || s_blend != NULL || s_srm != NULL) {
        pocketjs_ppa_deinit();
    ...
    esp_err_t result = register_client(PPA_OPERATION_FILL, &s_fill);
    if (result == ESP_OK) {
        result = register_client(PPA_OPERATION_BLEND, &s_blend);
    }
    if (result == ESP_OK) {
        result = register_client(PPA_OPERATION_SRM, &s_srm);
    }
    if (result != ESP_OK) {
        ESP_LOGE(
            TAG,
    ...
    s_fill_error_logged = false;
    s_blend_error_logged = false;
    s_srm_error_logged = false;
...
{
    if (s_srm != NULL) {
        (void)ppa_unregister_client(s_srm);
        s_srm = NULL;
    }
    if (s_blend != NULL) {
        (void)ppa_unregister_client(s_blend);
        s_blend = NULL;
    }
    if (s_fill != NULL) {
        (void)ppa_unregister_client(s_fill);
        s_fill = NULL;
    }
}
...
)
{
    ...
    const ppa_fill_oper_config_t operation = {
        ...
        // PPA's fixed fill pixel is always supplied as ARGB components. Passing
        // the packed RGB565 word here makes its two bytes look like B/G values.
        .fill_argb_color = rgb565_to_argb8888(color),
        .mode = PPA_TRANS_MODE_BLOCKING,
    };
    const esp_err_t result = ppa_do_fill(s_fill, &operation);
    if (result != ESP_OK) {
        log_operation_failure_once("fill", result, &s_fill_error_logged);
        return 0;
    }
    return 1;
}
...
)
{
    ...
    if (s_blend == NULL ||
        !surface_is_valid(destination, destination_pixels, width, height) ||
        !surface_is_valid(mask, mask_len, width, height) ||
        !rect_is_valid(width, height, x, y, rect_width, rect_height)) {
        return 0;
    }
    ...
    const esp_err_t result = ppa_do_blend(s_blend, &operation);
    if (result != ESP_OK) {
        log_operation_failure_once("blend", result, &s_blend_error_logged);
        return 0;
    }
    return 1;
}
